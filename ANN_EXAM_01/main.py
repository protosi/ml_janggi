...
#log_lik = tf.losses.mean_squared_error(Y, action_pred)
...
for step in range(max_num_episodes):
    ...
    # Print status
    print("[Episode {}] Reward: {} Loss: {}".format(step, reward_sum, l))
    
    if np.mean(EPISODE_100_REWARD_LIST) >= 195:
        print("Game Cleared within {} steps with the average reward: {}".format(step, np.mean(EPISODE_100_REWARD_LIST)))
        break
